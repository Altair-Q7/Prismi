...
// Level 1: 9 rings = same distractor colour, 1 ring = target colour
// Level 2: 3 rings colour A, 3 rings colour B, 3 rings colour C, 1 ring target
...
int  targetRing      = 0;            // which ring holds the target colour
...
void handleSerial() {
  ...
  if (cmd == "start") {
    ...
    if (d >= 1 && d <= 3) {
      difficulty = d;
      const char* levelNames[] = {"Easy (odd one out)", "Medium (3-3-3-1)", "Hard (all unique)"};
...
void newRound() {
  ...
  // Pick a random target ring
  targetRing = random(NUM_RINGS);
  // Target colour is the ring's own colour (always)
  int targetColorIdx = targetRing;

  // Assign colours to all rings according to difficulty
  if (difficulty == 1) {
    // Level 1: 9 rings = one distractor colour, 1 ring = target colour
    int distractor;
    do {
      distractor = random(NUM_RINGS);
    } while (distractor == targetColorIdx);

    for (int i = 0; i < NUM_RINGS; i++) {
      ringColorIdx[i] = (i == targetRing) ? targetColorIdx : distractor;
    }
  }
  else if (difficulty == 2) {
    // Level 2: 3 rings colour A, 3 rings colour B, 3 rings colour C, 1 ring target
    // Choose three distractor colours distinct from target and each other
    int d1, d2, d3;
    ...
    // Create an array of 10 colour assignments, then shuffle except target
    int assignments[10];
    assignments[0] = d1; assignments[1] = d1; assignments[2] = d1;
    assignments[3] = d2; assignments[4] = d2; assignments[5] = d2;
    assignments[6] = d3; assignments[7] = d3; assignments[8] = d3;
    assignments[9] = targetColorIdx;

    // Shuffle the first 9 elements (the distractor slots)
    for (int i = 0; i < 9; i++) {
      int j = random(9);
    ...
    // Place the target ring at its fixed position
    for (int i = 0; i < NUM_RINGS; i++) {
      if (i == targetRing) {
        ringColorIdx[i] = targetColorIdx;
      } else {
        // Assign from shuffled distractor pool, skipping the target position
        int idx = (i < targetRing) ? i : i - 1;
        ringColorIdx[i] = assignments[idx];
      }
    }
  }
  else { // difficulty == 3
...
void onCorrect() {
  ...
  int targetColorIdx = ringColorIdx[targetRing];
...
void onWrong(int pressed) {
  gameState = FEEDBACK;
  #if SERIAL_DEBUG
  Serial.printf("Wrong! Pressed %s, needed %s\n", COLORS[pressed].name, COLORS[ringColorIdx[targetRing]].name);
  #endif
  ...
  lightAllRings();
  if (oledAvailable) oledPrompt(COLORS[ringColorIdx[targetRing]].name, currentRound, maxRounds, score);
  roundStart = millis();
  gameState = WAITING;
}
...
void printHelp() {
  Serial.println();
  Serial.println("╔══════════════════════════════╗");
  Serial.println("║   PRISMI v1.7  Serial Cmd    ║");
  Serial.println("╠══════════════════════════════╣");
